...
export default function ForgotPassword() {
  const [sendOtpBtn, setSendOtpBtn] = useState({text: "Send OTP",bg: "blue",});
  const [otpInput, setOtpInput] = useState(false);
  const [verifyBtn, setVerifyBtn] = useState({text: "Verify OTP",bg: "Blue",});
  const [otpVerification, setotpVerification] = useState(false);
  const [submitBtn, setSubmitBtn] = useState({ text: "Change Password", bg: "Green" });
  const [showPassword, setShowPassword] = useState(false);

  const [data, setdata] = useState({ email: "", inputOTP:"", password: "", cpassword: "" });
  ...
  const changePass = async(e) => {
    ...
    if(password != cpassword){
      const notify = () => toast.warn("password and confirm password not matched");
      notify();
      return;
    }
      ...
      <div className="forgot-password-page">
        <div className="forgot-password-pop">
          <div className="email-field">
            ...
            <input type="email" name="email" id="femail" onChange={handleData}/>
            <button  style={{margin:"10px", backgroundColor: `${sendOtpBtn.bg}` }} onClick={sendOTP}>{sendOtpBtn.text}</button>
          </div>
          ...
          {otpInput ? (
            ...
            <button style={{ backgroundColor: `${verifyBtn.bg}` }} onClick={verifyOTP}>{verifyBtn.text}</button>
          </div>
          ) : ("")}
          ...
          <div className="change-password-field" style={{textAlign:"right", marginTop:"23px"}}>
            <button style={{ backgroundColor: `${submitBtn.bg}` }} onClick={changePass}>{submitBtn.text}</button>
          </div>
          </>
